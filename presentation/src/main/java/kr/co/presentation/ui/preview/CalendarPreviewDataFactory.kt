...
object CalendarPreviewDataFactory {
    // 년도 헤더 + 12 개월
     ...
     *
     * @param year 대상 연도.
     * @return 연도 및 월별 아이템 리스트.
     */
    fun createGridItemsOf(year: Year): List<YearMonthItem> {
        val result = ArrayList<YearMonthItem>(COUNT_OF_CALENDAR_ITEM)
        ...
        for (startMonth in quarterStartMonths) {
            for (i in 0..2) {
                ...
                result.add(createMonthItem(YearMonth.of(year.value, monthValue)))
            }
        }

        result.add(YearItem(year))
        return result
    }
     ...
     */
    fun createMonthItems(yearMonth: YearMonth): List<MonthItem> {
        return List(COUNT_OF_MONTH) { index ->
            createMonthItem(yearMonth.minusMonths(index.toLong()))
        }
    }

    /**
     * 특정 연월에 해당하는 월 아이템을 생성합니다.
     *
     * @param yearMonth 대상 연월.
     * @return 생성된 월 아이템.
     */
    fun createMonthItem(yearMonth: YearMonth): MonthItem {
        return MonthItem(
            yearMonth = yearMonth,
    ...
    /**
     * 특정 연월의 날짜 아이템 리스트를 생성합니다.
     * 이전 달, 현재 달, 다음 달의 날짜를 포함하여 항상 42개의 날짜 아이템을 생성합니다.
     *
     * @param yearMonth 대상 연월.
     * @return 42개의 날짜 아이템 리스트.
     */
    private fun createDays(yearMonth: YearMonth): ImmutableList<DayItem> {
        val monthDays = ArrayList<DayItem>(COUNT_OF_ALL_DAYS)
        ...
        return monthDays.toImmutableList()
    }
}
