...
import kr.co.data.local.datastore.UserSyncPrefsDataStore
...
@Singleton
class UserSyncDataStoreProvider @Inject constructor(
    @param:ApplicationContext private val context: Context,
    private val firebaseAuth: FirebaseAuth,
    private val pathProvider: LocalStoragePathProvider
) {
    private val dataStores = ConcurrentHashMap<String, UserSyncPrefsDataStore>()
    private val scopes = ConcurrentHashMap<String, CoroutineScope>()

    private fun getDataStoreFile(uid: String) =
        context.preferencesDataStoreFile(pathProvider.getUserSyncDataStoreName(uid))

    fun getDataStore(): UserSyncPrefsDataStore {
